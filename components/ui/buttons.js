...
export default function Button({ children, href, size, type, color }) {
  ...
  if (color === "white") {
    ...
    btnTypeClass =
      type === "solid"
        ? `bg-custom-primary text-on-primary-color hover:bg-custom-primary/80`
        : type === "outline"
        ? `text-custom-primary border-2 border-primary-color hover:bg-custom-primary/20`
        : type === "link"
        ? `text-custom-primary hover:text-custom-primary/60 !px-0`
        : `bg-custom-primary text-on-primary-color hover:bg-primary-color/80`;
  }
